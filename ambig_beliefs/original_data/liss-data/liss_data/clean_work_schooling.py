...
def _clean_logically_work_schooling(panel):
    ...
    # Add row category codes for occupation variable
    temp = pd.read_pickle(OUT_DATA_LISS / "work_schooling_raw_cat_codes.pickle").copy()
    temp["occupation_isco_08"] = (
        temp["occupation_isco_08_labels"].replace({-9: np.nan}).astype(float)
    )
    temp = temp.set_index(["personal_id", "year"])
    out = out.join(temp["occupation_isco_08"], on=["personal_id", "year"])
