...
with open(IN_SPECS_LISS / "data_sets_specs.yaml") as f:
    dir_dict = yaml.full_load(f)


PARAMETRIZATION = []
# for directory in ["006-work-and-schooling"]:
for directory in dir_dict:
    for file_format in FILE_FORMATS_LISS:
        ...
        if directory == "xxx-ambiguous-beliefs" and file_format in ["pickle", "csv"]:
            target.update(
                {
                    f"event_properties_{file_format}": OUT_DATA_LISS
                    / "ambiguous_beliefs"
                    / f"event_properties.{file_format}",
                    f"choice_properties_{file_format}": OUT_DATA_LISS
                    / "ambiguous_beliefs"
                    / f"choice_properties.{file_format}",
                    f"choices_{file_format}": OUT_DATA_LISS
                    / "ambiguous_beliefs"
                    / f"choices.{file_format}",
                    f"baseline_matching_probs_{file_format}": OUT_DATA_LISS
                    / "ambiguous_beliefs"
                    / f"baseline_matching_probs.{file_format}",
                    f"indices_{file_format}": OUT_DATA_LISS
                    / "ambiguous_beliefs"
                    / f"indices.{file_format}",
                    f"choices_pay_out_info_{file_format}": OUT_DATA_LISS
                    / "ambiguous_beliefs"
                    / f"choices_pay_out_info.{file_format}",
                }
            )
...
def prepare_panel(out_format, file_paths, data_set_name):
    ...
    for file_path in file_paths:
        ...
        if file_name not in rename_df:
            ...
            # Load raw category codes
            data_raw_cat_codes = read_stata(
                file_path,
            ...
            data_raw_cat_codes = data_raw_cat_codes.rename(columns=rename_dict)
            ...
            if specs["multiple_files_per_year"]:
                ...
                # HOTFIX: Lets try to find a better way of assigning years!!
                if file_name == "cv10c_EN_1.0p.dta":
                    year = 2010
                else:
                    year = (
                        data[data["date_fieldwork"].notnull()]["date_fieldwork"]
            ...
            data_set_list_raw_cat_codes.append(data_raw_cat_codes)

    save_panel(
        panel=pd.concat(data_set_list_raw_cat_codes, ignore_index=True, sort=False),
        file_name=specs["file_name"] + "_raw_cat_codes",
        out_format="pickle",
    )
