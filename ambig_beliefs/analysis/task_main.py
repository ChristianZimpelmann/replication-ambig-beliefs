...
class ModelWithLikelihood:
    ...
    #     """
    #     Calculate std errors for model
    #     """

    #     def _unpack_paras_df(paras_df):
    #         paras = paras_df["value"].values
    #         paras = self._unpack_para_vec(paras)
    #         return paras

    #     def neg_log_likeli_list(x, agent):
    #         paras = _unpack_paras_df(x)
    #         if self.least_squares_estimation:
    #             return -agent.least_squares_objective_ind(paras)
    #         else:
    #             return -agent.eval_likelihood_ind(paras)

    #     def neg_log_likeli(x, agent):
    #         paras = _unpack_paras_df(x)
    #         if self.least_squares_estimation:
    #             return -agent.least_squares_objective_ind(paras).sum()
    #         else:
    #             return -agent.eval_likelihood(paras)

    #     res_estimation = pd.read_pickle(
    #         OUT_ANALYSIS / self.model_name / "opt_diff_evolution" / "results.pickle"
    #     )
    #     # Build parameter dataframe for estimagic
    #     paras = pd.DataFrame(
    #         {},
    #         columns=[
    #             "description",
    ...
    #         ],
    #     )
    #     parameters = self.model_specs["model_parameters"]

    #     # Add error parameters for least_squares estimation which is treated as ml estimation
    #     if self.least_squares_estimation:
    #         parameters += [
    #             {
    #                 "description": "Strength of random component (Fechner error)",
    #                 "fixed": False,
    #                 "heterogeneous_over_waves": False,
    #                 "name": "theta",
    #                 "symbol": "\u03b8",
    #                 "lower_bound": 0,
    #                 "upper_bound": np.nan,
    ...
    #             },
    #             {
    #                 "description": "Probability of a random choice",
    #                 "fixed": True,
    #                 "heterogeneous_over_waves": False,
    #                 "name": "omega",
    #                 "symbol": "\u03c9",
    ...
    #             self.upper_bounds,
    #         ) = get_start_values_bounds(self.model_parameters)
    #     for mp_init in parameters:
    #         if mp_init["heterogeneous_over_waves"]:
    #             for wave in self.used_waves:
    #                 adjusted_mp_init = mp_init.copy()
    #                 adjusted_mp_init["name"] += "_w" + str(wave)
    #                 paras.loc[adjusted_mp_init["name"]] = [
    #                     adjusted_mp_init["description"],
    #                     adjusted_mp_init["fixed"],
    #                     adjusted_mp_init["heterogeneous_over_waves"],
    #                     adjusted_mp_init["symbol"],
    #                     adjusted_mp_init["lower_bound"],
    #                     adjusted_mp_init["upper_bound"],
    #                     adjusted_mp_init["start_value"],
    #                 ]

    #         else:
    #             paras.loc[mp_init["name"]] = [
    #                 mp_init["description"],
    #                 mp_init["fixed"],
    #                 mp_init["heterogeneous_over_waves"],
    #                 mp_init["symbol"],
    #                 mp_init["lower_bound"],
    #                 mp_init["upper_bound"],
    #                 mp_init["start_value"],
    #             ]
    #     fixed_parameters = [mp["name"] for mp in parameters if mp["fixed"]]
    #     paras = paras.drop(fixed_parameters)

    #     # Calculate std errors for each agent
    #     standard_errors = pd.DataFrame(
    #         {}, index=self.agents, columns=[c for c in paras.index] + ["pos_def"]
    #     )
    #     for agent_id, agent in self.agents.items():
    #         if self.least_squares_estimation:
    #             res_estimation["theta"] = res_estimation["std_error"]
    #         paras["value"] = res_estimation.loc[agent_id].astype(float)
    #         hessian_mat = hessian(neg_log_likeli, paras, func_kwargs={"agent": agent})
    #         jacobian_mat = first_derivative(
    #             neg_log_likeli_list, paras, func_kwargs={"agent": agent}
    #         )
    #         try:
    #             np.linalg.cholesky(hessian_mat)
    #             pos_def = True
    #         except np.linalg.LinAlgError:
    #             pos_def = False
    #         try:
    #             standard_errors.loc[agent_id] = se_from_cov(
    #                 cov_sandwich(jacobian=jacobian_mat, hessian=hessian_mat)
    #             ).tolist() + [pos_def]
    #         except:
    #             standard_errors.loc[agent_id] = [np.nan] * len(paras.index) + [pos_def]

    #     # Save results
    #     standard_errors.to_pickle(
    #         OUT_ANALYSIS
    #         / self.model_name
    #         / self.opt_settings_name
    #         / ("standard_errors" + ".pickle")
    #     )
