...
def _additional_variables(panel):
    good_cols = ["feel_calm", "feel_happy"]
    bad_cols = [
        "feel_anxious",
        "feel_down",
        "feel_depressed",
    ]
    panel["mhi5"] = _create_mhi5(in_data=panel, good_cols=good_cols, bad_cols=bad_cols)
    return panel


def _create_mhi5(in_data, good_cols, bad_cols):
    """Create mhi 5. TOBE PLACED in utils!"""
    data = in_data.copy()
    for col in good_cols + bad_cols:
        assert data[col].cat.ordered

    for col in bad_cols:
        data[col] = pd.Categorical(
            data[col], categories=list(reversed(data[col].cat.categories)), ordered=True
        )

    out = functools.reduce(
        lambda x, y: x + y,
        [data[col].cat.codes.replace({-1: np.nan}) for col in good_cols + bad_cols],
    )
