...
def clean_ambiguous_beliefs(data, file_format):
    ...
    rename_df = pd.read_csv(
        IN_SPECS_LISS / "xxx-ambiguous-beliefs_renaming.csv", sep=";",
    )
...
def make_numeracy_vars(var_to_sol, data, var_name):
    ...
    if var_name == "basic_numeracy":
        data["basic_numeracy"] = temp.sum(axis=1) / temp.count(axis=1).max()
        data[var_name + "_stdzd"] = std_normalise(data["basic_numeracy"])
    else:
...
def add_risk_pref(data):
    ...
    data["quantitative_risk_q"] = data["ce_for_300eur_50pct"].map(
        {float(5 + (10) * i): (i + 1) for i in range(32)}
    )
